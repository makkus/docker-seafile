...
then
		...
		then
				...
				then
						...
						sed -i -e "s/enabled = false/enabled = true/" /opt/seafile/pro-data/seafevents.conf
						sed -i -e "s/index_office_pdf.*/index_office_pdf = true/" /opt/seafile/pro-data/seafevents.conf
				fi
		fi
fi
